...
Controller_t createController(QueueHandle_t queue, EventGroupHandle_t measureEventGroup, EventGroupHandle_t dataReadyEventGroup, EventGroupHandle_t loraReadyEventGroup, SemaphoreHandle_t sensorSemaphore, CO2_t CO2_sensor, Temperature_Humidity_t Temp_sensor, Sound_t Sound_sensor, Servo_t servo){
...
static void servoController(Controller_t self){
	...
	if (CO2Data < 650 ){
		closeWindowServo(self->servo);
	}
	else if (CO2Data > 650 && CO2Data < 1000){
		halfOpenWindowServo(self->servo);
	}
	else{
		openWindowServo(self->servo);
	}
...
static void dataPackageController(Controller_t self){
	...
	dph_SetHumidity(getHum(self->Temp_sensor), package);
	dph_SetTemperature(getTemp(self->Temp_sensor), package);
